function [frames, video_object] = loadSpecificFrames(videoPath,frameNumber,fig)
...
elseif nargin == 1
    frameNumber = 1 : 1 :  round(video_object.Duration*video_object.FrameRate);
end

video_object.NumFrames = length(frameNumber);
...
% Number of frames
frames = cell(1,length(frameNumber));  % Initialize the cell array for storing frames


for current_frame = frameNumber
    try
