function imageExported = uiGetImage()
Image = [] ; % intitialized
%% UI configuration
%%%%%%%%%%%%%%%%%%%%
fig = uifigure('Name', 'Image import tool');
fig.Color =  [1 1 1];
...
bxl.Items = {'Select your images files'};
...
%%%%%%%%%%%%%%%%%%%%
uiwait(fig);
for ii = 1 : length(imageExported)
    formatSpec = 'image :" %s " is import to the workspace\n';
    fprintf(formatSpec,imageExported(ii).path);
end
delete(fig)
...
% search
    function search(bxl,btn_load,btn_clear,fig)
        [f_,pathname] = uigetfile({'*.jpg','Image (*.jpg)';...
            '*.*'  ,'Tous les Fichiers (*.*)';},...
        ...
        if isempty(Image) == 1
            ...
            lbeg = length(Image);
            for i = 1 : length(f_)
                Image(i + lbeg).path = f{i};
                Image(i + lbeg).name = f_{i};
            end
        end

        for i = 1 : length(Image)
            items(i) = string(Image(i).path) ;
        end
...
% files_import
    function files_import(fig,btn_load,btn_clear,btn_search)
        ...
        for videoidx = 1 : length(Image)
            imageExported(videoidx) = importImage(Image(videoidx).path,Image(videoidx).name,fig);
        end
        uiresume(fig);
    end
